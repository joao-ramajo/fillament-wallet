...
class CreditCardStatementService
{
    ...
    {
        $statement = CreditCardStatement::find($statementId);
    ...
    {
        $totalAmount = (int) Expense::where('credit_card_statement_id', $statement->id)
    ...
    public function splitInstallments(int $amount, int $installmentTotal): array
    {
        if ($installmentTotal < 1) {
            throw new DomainException('A quantidade de parcelas deve ser maior que zero.');
        }
    ...
    private function determineStatus(?\DateTimeInterface $paidAt, CarbonImmutable $closingAt): string
    {
        ...
        return now()->startOfDay()->gt($closingAt)
